...
ETNube::ETNube() {
    // this->cloudData = std::vector<Data>;
}
...
void ETNube::updateLocation(std::string owner, std::string equipment, double x, double y){
    ...
    if ((location=getLocation(owner,equipment))==NULL){
        ...
        Data data = Data(owner, equipment, location);
        cloudData.insert(cloudData.end(),data);
    }
    location->setX(x);
    location->setY(y);
}
std::string ETNube::getFindMy(std::string owner){
    ...
    for (int i = 1 ; i < cloudData.size() ; i++){
        ...
        if (data.ownerName == owner){
            std::string buffer = (data.equipmentName + ": ("
            + std::to_string((int)getLocation(owner, data.equipmentName)->x()) + " , "
                                  + std::to_string((int)getLocation(owner, data.equipmentName)->y()) + ")\n");
