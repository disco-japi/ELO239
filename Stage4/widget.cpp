...
{
    nube = new ETNube;
    setWindowTitle("EloTelTag Simulation in C++ and Qt: Stage 4");
    resize(1000, 700);
...
// NUEVO: Implementar toggleTraza
void Widget::toggleTraza(const QString &nombreEquipo)
{
    if (equiposMap.contains(nombreEquipo)) {
        Equipo *eq = equiposMap[nombreEquipo];
        eq->toggleTraza();

        // Actualizar la vista correspondiente
        for (EloTelTagView *v : tagsViews) {
            if (v->getModel() == eq) {
                v->updateTraza();
                break;
            }
        }
        for (TabletView *v : tabletsViews) {
            if (v->getModel() == eq) {
                v->updateTraza();
                break;
            }
        }
        for (CellularView *v : celularesViews) {
            if (v->getModel() == eq) {
                v->updateTraza();
                break;
            }
...
void Widget::cargarConfiguracion(const QString &filePath)
{
    ...
    for (int i = 0; i < numPersonas; ++i)
    {
        ...
        if (hasTablet == 1)
        {
            ...
            equiposMap["Tablet"] = tab; // NUEVO: Registrar en mapa
